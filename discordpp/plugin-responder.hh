			...
			handlers.insert(
					{
							"READY",
							[this](json data){
								id_ = boost::lexical_cast<snowflake>(data["user"]["id"].get<std::string>());
							}
					}
			);
		}
			...
			handlers.insert(
					{
							...
							[this, command, handler](json msg){
								...
								if(boost::lexical_cast<snowflake>(msg["author"]["id"].get<std::string>()) != id_ && msg["content"].get<std::string>().rfind(prefix + command, 0) == 0){
									// Run the response
									handler(msg);
								}
							}
